...
// Add this ref for the player section
const playerSection = ref(null);

// Add a new ref for playback loading state
...
// Add these methods after the pagination methods and before selectStation
const updateVolume = () => {
  if (audioStream.value) {
    // Ensure volume is between 0 and 1
    const newVolume = Math.max(0, Math.min(1, volume.value));
...
const toggleMute = () => {
  if (audioStream.value) {
    if (isMuted.value) {
      ...
      audioStream.value.volume = previousVolume.value;
      isMuted.value = false;
    } else {
      // Mute: save current volume and set to 0
      previousVolume.value = volume.value;
      volume.value = 0;
      audioStream.value.volume = 0;
      isMuted.value = true;
    }
  }
};

// Update the selectStation function to include volume initialization
const selectStation = async (station) => {
  try {
    ...
    audioStream.value.volume = isMuted.value ? 0 : volume.value; // Initialize with current volume
    ...
    audioStream.value.addEventListener('playing', () => {
      ...
      error.value = null;
      isPlaybackLoading.value = false;  // Clear loading state on successful play
    });
    ...
    audioStream.value.addEventListener('error', (e) => {
      ...
      isPlaying.value = false;
      isPlaybackLoading.value = false;  // Clear loading state on error
    });
